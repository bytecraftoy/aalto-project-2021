...
router.route('/tag/:proj').get(async (req: Request, res: Response) => {
...
router.route('/tag/:proj/:node').post(async (req: Request, res: Response) => {
    const projId = parseInt(req.params.proj);
    ...
    let tag: ITag | null = req.body.tag_id;
    if (!tag) {
        const label: string | null = req.body.tag_label;
        ...
        if (label) {
            ...
            if (insertTagQuery.rowCount == 1) {
                const tagId = insertTagQuery.rows[0]?.id;
                tag = {
                    project_id: projId,
                    id: tagId,
                    label: label,
    ...
    if (tag) {
        ...
        if (projId && nodeId && tag) {
            const insertTaggedNodeQuery = await db.query(
                'INSERT INTO tagged_nodes (node_id, tag_id, project_id) VALUES ($1, $2, $3) RETURNING node_id, tag_id, project_id',
                [nodeId, tag.id, projId]
            );

            if (insertTaggedNodeQuery.rowCount == 1) {
                const retTaggedNode: ITaggedNode | null =
                    insertTaggedNodeQuery.rows[0];
                if (retTaggedNode) {
...
router
    ...
    .delete(async (req: Request, res: Response) => {
        ...
        return res
            .status(401)
            .json({ error: 'failed to remove tag from node' });
    });
