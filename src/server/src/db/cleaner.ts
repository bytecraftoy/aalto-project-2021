...
export async function doClean(client: ClientBase) {
    for (const statement of await generateDropStatementsForMaterializedViews(
        client
    )) {
    ...
    for (const statement of await generateDropStatementsForBaseTypes(
        client,
        true
    )) {
        await client.query(statement, []);
...
async function queryForStringList(
    client: ClientBase,
    text: string,
    ...values: unknown[]
): Promise<string[]> {
    return (await client.query({ text, values, rowMode: 'array' })).rows.map(
        (row) => row[0]
    );
}
 ...
 */
async function generateDropStatementsForExtensions(
    client: ClientBase
): Promise<string[]> {
    const statements: string[] = [];

    if (await extensionsTableExists(client)) {
        const extensionNames = await queryForStringList(
            client,
            'SELECT e.extname ' +
                'FROM pg_extension e ' +
 ...
 * @throws SQLException when the clean statements could not be generated.
 */
async function generateDropStatementsForSequences(
    client: ClientBase
): Promise<string[]> {
    const statements: string[] = [];
    const sequenceNames = await queryForStringList(
        client,
    ...
    for (const sequenceName of sequenceNames) {
        // XXX: injection
        statements.push(
            'DROP SEQUENCE IF EXISTS ' + namespace + '.' + sequenceName
        );
    }
 ...
 * @throws SQLException when the clean statements could not be generated.
 */
async function generateDropStatementsForBaseTypes(
    client: ClientBase,
    recreate: boolean
): Promise<string[]> {
    ...
    for (const row of rows) {
        statements.push(
            'DROP TYPE IF EXISTS ' + namespace + '.' + row.typname + ' CASCADE'
        );
    }
 ...
 * @throws SQLException when the clean statements could not be generated.
 */
async function generateDropStatementsForRoutines(
    client: ClientBase
): Promise<string[]> {
    const rows: any[] = (
        await client.query(
 ...
 * @throws SQLException when the clean statements could not be generated.
 */
async function generateDropStatementsForEnums(
    client: ClientBase
): Promise<string[]> {
    const enumNames = await queryForStringList(
        client,
        "SELECT t.typname FROM pg_catalog.pg_type t INNER JOIN pg_catalog.pg_namespace n ON n.oid = t.typnamespace WHERE n.nspname = $1 and t.typtype = 'e'",
        namespace
    );
 ...
 * @throws SQLException when the clean statements could not be generated.
 */
async function generateDropStatementsForDomains(
    client: ClientBase
): Promise<string[]> {
    const domainNames = await queryForStringList(
        client,
 ...
 * @throws SQLException when the clean statements could not be generated.
 */
async function generateDropStatementsForMaterializedViews(
    client: ClientBase
): Promise<string[]> {
    const viewNames = await queryForStringList(
        client,
 ...
 * @throws SQLException when the clean statements could not be generated.
 */
async function generateDropStatementsForViews(
    client: ClientBase
): Promise<string[]> {
    const viewNames = await queryForStringList(
        client,
    ...
    for (const domainName of viewNames) {
        statements.push(
            'DROP VIEW IF EXISTS ' + namespace + '.' + domainName + ' CASCADE'
        );
    }
...
async function generateDropStatementsForTables(
    client: ClientBase
): Promise<string[]> {
    const tableNames = await queryForStringList(
        client,
